...
import (
	...
	"github.com/jonbodner/coverlight/browser"
...
func htmlOutput(profile, outfile, mode string) error {
	...
	case "light":
		htmlTemplate = template.Must(template.New("html").Funcs(template.FuncMap{
			"colors": colors(rgbLight),
		}).Parse(tmplHTMLLight))
	case "dark":
		htmlTemplate = template.Must(template.New("html").Funcs(template.FuncMap{
			"colors": colors(rgbDark),
		}).Parse(tmplHTMLDark))
	default:
...
// rgbLight returns an rgb value between mid-gray and black for the specified coverage
// value between 0 (no coverage) and 10 (max coverage).
func rgbLight(n int) string {
	if n == 0 {
		return ".cov0 { font-style: normal; color: white; background-color: black; }"
	}
	return fmt.Sprintf(".cov%v {color: black; font-style: normal; font-weight: bold; }", n)
}

// rgb returns an rgb value for the specified coverage value
// between 0 (no coverage) and 10 (max coverage).
func rgbDark(n int) string {
	if n == 0 {
		return ".cov0 { color: rgb(192, 0, 0) }" // Red
	}
	// Gradient from gray to green.
	r := 128 - 12*(n-1)
	g := 128 + 12*(n-1)
	b := 128 + 3*(n-1)
	return fmt.Sprintf(".cov%v { color: rgb(%v, %v, %v) }", n, r, g, b)
}

// colors generates the CSS rules for coverage colors.
func colors(rgb func(int) string) func() template.CSS {
	return func() template.CSS {
		var buf strings.Builder
		for i := 0; i < 11; i++ {
			fmt.Fprintf(&buf, "%s\n", rgb(i))
		}
